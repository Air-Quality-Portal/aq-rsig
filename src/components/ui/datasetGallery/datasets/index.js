export const galleryData = {
  satellite: [
    {
      id: 'omi-no2-2d',
      name: 'OMI Tropospheric NO₂ Column Data',
      thumbnailUrl: '/path/to/thumbnail1.jpg',
      type: 'raster',
      description:
        'Berkeley High Resolution(BEHR) Ozone Monitoring Instrument (OMI) Integrated Column Amount of NO2 in the Tropopsphere.',
      units: 'molec/cm²',
      rescale_values: [100000000000000.0, 6400000000000000.0],
      colormap: 'viridis',
      info: 'https://amt.copernicus.org/articles/14/455/2021/',
      time_interval: 'Daily',
      start_date: '2014-06-01T00:00:00Z',
      end_date: '2014-09-30T00:00:00Z',
      stops: ['#440154', '#3b528b', '#21908d', '#5dc962', '#fde725'],
      is_animatable: true,
    },
    {
      id: 'TROPESS_reanalysis_mon_emi_O3_anth',
      name: 'TROPESS O3 Emissions',
      type: 'netcdf-2d',
      thumbnailUrl: '/path/to/thumbnail3.jpg',
      description:
        "The TROPESS Chemical Reanalysis O3 Spread Monthly 3-dimensional Product contains the ozone ensemble spread, a measure of data assimilation analysis uncertainty. The data are part of the Tropospheric Chemical Reanalysis v2 (TCR-2) for the period 2005-2021. TCR-2 uses JPL's Multi-mOdel Multi-cOnstituent Chemical (MOMO-Chem) data assimilation framework that simultaneously optimizes both concentrations and emissions of multiple species from multiple satellite sensors.",
      units: 'kg m-2 s-1',
      rescale_values: [0, 100],
      colormap: 'reds',
      info: 'https://data.nasa.gov/dataset/tropess-chemical-reanalysis-ozone-spread-monthly-3-dimensional-product-v1-trpscro3sm3d-at-',
      time_interval: 'Monthly',
      start_date: '2005-01-01T00:00:00+00:00',
      end_date: '2021-12-31T00:00:00+00:00',
      stops: ['#fff5f0', '#fcbba1', '#fc9272', '#fb6a4a', '#de2d26', '#a50f15'],
      is_animatable: true,
    },
    {
      id: 'JPL-o3-hourly-v1',
      name: 'JPL MOMO Chem O3',
      type: 'raster',
      thumbnailUrl: '/path/to/thumbnail3.jpg',
      description:
        "This dataset provides hourly atmospheric composition data simulated using the MOMO-Chem model, developed by NASA's Jet Propulsion Laboratory (JPL). It offers insights into various atmospheric constituents, including ozone (O₃), carbon monoxide (CO), nitrogen dioxide (NO₂), sulfur dioxide (SO₂), and fine particulate matter (PM₂.₅).",
      units: 'molecules/cm²',
      colormap: 'rdbu_r',
      rescale_values: [0, 80],
      info: 'https://geoschem.github.io/index.html',
      time_interval: 'hourly',
      start_date: '2020-07-19 14:00:00+00',
      end_date: '2020-07-31 23:59:59+00',
      stops: ['#67001f', '#d6604d', '#f7f7f7', '#4393c3', '#053061'],
      is_animatable: true,
    },
    {
      id: 'geos-cf-ana-co',
      name: 'GEOS-CF CO (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-co-thumb.jpg',
      description: 'GEOS-CF analysis for Carbon Monoxide (CO).',
      units: 'mol/mol',
      rescale_values: [0.1, 0.2],
      colormap: 'ylorrd',
      collection_id: 'geos-cf-ana',
      asset_name: 'CO',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
        {
      id: 'geos-cf-fcst-co',
      name: 'GEOS-CF CO (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-co-thumb.jpg',
      description: 'GEOS-CF forecast for Carbon Monoxide (CO).',
      units: 'mol/mol',
      rescale_values: [0.1, 0.2],
      colormap: 'ylorrd',
      collection_id: 'geos-cf-fcst',
      asset_name: 'CO',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-ana-o3',
      name: 'GEOS-CF O₃ (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-o3-thumb.jpg',
      description: 'GEOS-CF analysis for Ozone (O₃).',
      units: 'mol/mol',
      rescale_values: [0.01, 0.06],
      colormap: 'blues',
      collection_id: 'geos-cf-ana',
      asset_name: 'O3',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
        {
      id: 'geos-cf-fcst-o3',
      name: 'GEOS-CF O₃ (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-o3-thumb.jpg',
      description: 'GEOS-CF forecast for Ozone (O₃).',
      units: 'mol/mol',
      rescale_values: [0.01, 0.06],
      colormap: 'blues',
      collection_id: 'geos-cf-fcst',
      asset_name: 'O3',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-ana-no2',
      name: 'GEOS-CF NO₂ (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-no2-thumb.jpg',
      description: 'GEOS-CF analysis for Nitrogen Dioxide (NO₂).',
      units: 'mol/mol',
      rescale_values: [0, 5],
      colormap: 'greens',
      collection_id: 'geos-cf-ana',
      asset_name: 'NO2',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-fcst-no2',
      name: 'GEOS-CF NO₂ (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-no2-thumb.jpg',
      description: 'GEOS-CF forecast for Nitrogen Dioxide (NO₂).',
      units: 'mol/mol',
      rescale_values: [0, 5],
      colormap: 'greens',
      collection_id: 'geos-cf-fcst',
      asset_name: 'NO2',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },    
    {
      id: 'geos-cf-ana-so2',
      name: 'GEOS-CF SO₂ (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-so2-thumb.jpg',
      description: 'GEOS-CF analysis for Sulfur Dioxide (SO₂).',
      units: 'mol/mol',
      rescale_values: [0, 2],
      colormap: 'viridis',
      collection_id: 'geos-cf-ana',
      asset_name: 'SO2',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-fcst-so2',
      name: 'GEOS-CF SO₂ (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-so2-thumb.jpg',
      description: 'GEOS-CF forecast for Sulfur Dioxide (SO₂).',
      units: 'mol/mol',
      rescale_values: [0, 2],
      colormap: 'viridis',
      collection_id: 'geos-cf-fcst',
      asset_name: 'SO2',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },    
    {
      id: 'geos-cf-ana-pm10',
      name: 'GEOS-CF PM₁₀ (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-pm10-thumb.jpg',
      description: 'GEOS-CF analysis for PM₁₀.',
      units: 'ug/m³',
      rescale_values: [0, 400],
      colormap: 'turbo',
      collection_id: 'geos-cf-ana',
      asset_name: 'PM10',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-fcst-pm10',
      name: 'GEOS-CF PM₁₀ (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-pm10-thumb.jpg',
      description: 'GEOS-CF forecast for PM₁₀.',
      units: 'ug/m³',
      rescale_values: [0, 400],
      colormap: 'turbo',
      collection_id: 'geos-cf-fcst',
      asset_name: 'PM10',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-ana-pm25',
      name: 'GEOS-CF PM₂.₅ (Analysis)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-pm25-thumb.jpg',
      description: 'GEOS-CF analysis for PM₂.₅.',
      units: 'ug/m³',
      rescale_values: [0, 200],
      colormap: 'turbo',
      collection_id: 'geos-cf-ana',
      asset_name: 'PM25',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },
    {
      id: 'geos-cf-fcst-pm25',
      name: 'GEOS-CF PM₂.₅ (Forecast)',
      type: 'raster',
      thumbnailUrl: '/path/to/geos-cf-pm25-thumb.jpg',
      description: 'GEOS-CF forecast for PM₂.₅.',
      units: 'ug/m³',
      rescale_values: [0, 200],
      colormap: 'turbo',
      collection_id: 'geos-cf-fcst',
      asset_name: 'PM25',
      time_interval: 'hourly',
      start_date: '2025-10-22 00:30:00+00',
      end_date: '2025-10-26 23:30:00+00',
      is_animatable: true,
    },    
  ],
  insitu: [
    {
      id: 'public.aqs_gases_metadata',
      name: 'AQS Stations',
      type: 'feature',
      thumbnailUrl: '/path/to/thumbnail1.jpg',
      description:
        'EPA Air Quality System (AQS) monitoring stations that measure surface-level gaseous pollutants, including NOx, O₃, and others.',
      info: 'https://data.nasa.gov/dataset/tropess-chemical-reanalysis-surface-total-nox-emissions-monthly-2-dimensional-product-v1-t-2206f',
      time_interval: 'yearly',
      start_date: '2017-01-01T00:00:00+00:00',
      end_date: '2022-12-31T00:00:00+00:00',
      is_animatable: false,
    },
  ],
  Lidar: [
    {
      id: 'calipso-point-cloud',
      name: 'CALIPSO 532 nm Backscatter (Level 1) - [June 30 2023]',
      type: 'point-cloud',
      url: 'https://rsig-point-cloud.s3.us-west-2.amazonaws.com/outputs3/calipso_2023-06-30_{i}/ept-tileset/tileset.json',
      thumbnailUrl: '/path/to/thumbnail1.jpg',
      description:
        'CALIPSO Level 1 data provides calibrated, geolocated lidar measurements of attenuated backscatter at multiple wavelengths. These observations are the foundation for higher-level cloud and aerosol products, enabling studies of atmospheric vertical structure and optical properties.',
      units: ' km⁻¹ sr⁻¹',
      info: 'https://catalog.data.gov/dataset/calipso-lidar-level-2-aerosol-profile-v4-51-06c1f',
      time_interval: '100minutes per scan',
      start_date: '2023-06-30T21:39:53',
      end_date: '2023-06-30T21:39:53',
      available_dates: ['2020-07-20T07:15:43Z', '2023-06-31T21:39:53Z'],
      is_animatable: false,
    },
  ],
};
